...
class HARTForT2I(PreTrainedModel):
    ...
    def __init__(self, config: HARTForT2IConfig):
        ...
        print(f'Patch nums tuple: {self.patch_nums}')
    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(self.patch_nums[:-1]):  # si: i-th segment
            ...
            # Decode intermediate image from current latent f_hat and store it
            intermediate_img = self.vae_proxy[0].fhat_to_img(f_hat).add_(1).mul_(0.5).detach().cpu()
            intermediate_imgs.append(intermediate_img)
        ...
        , intermediate_imgs)  # de-normalize, from [-1, 1] to [0, 1]
